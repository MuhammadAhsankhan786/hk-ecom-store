...
export default function Header() {
  ...
  // Compute live search suggestions only when query is >= 2 chars
  const liveResults = useMemo(() => {
    ...
    ).slice(0, 5) // Return top 5 instant recommendations
    ...
    <header className="sticky top-0 z-40 bg-white shadow-xs">
      ...
      <div className="border-b border-[#E8E5DE] py-3 sm:py-4">
        <div className="max-w-[1440px] mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-2 sm:gap-4">
          ...
          {/* Left Search Bar with Debounce & Live Suggestions (Desktop) */}
          <div className="hidden lg:block w-72 relative" ref={searchContainerRef}>
            ...
            {isFocused && debouncedSearchQuery.trim().length >= 2 && (
              <div className="absolute left-0 right-0 top-full mt-2 bg-white border border-[#E8E5DE] rounded-xl shadow-xl z-50 overflow-hidden">
                <div className="p-2 border-b border-[#F8F7F3] flex justify-between items-center text-[10px] text-[#6B6B6B] uppercase tracking-wider font-semibold">
                  <span>Live Matches</span>
                  <span>{liveResults.length} Found</span>
                </div>
                {liveResults.length > 0 ? (
                  <div className="divide-y divide-[#F8F7F3]">
                    {liveResults.map(p => (
                      <Link
                        key={p.id}
                        href={`/product/${p.id}`}
                    ...
                    <button
                      ...
                      className="w-full text-center py-2.5 text-[11px] font-semibold text-[#D4AF37] hover:bg-[#F8F7F3] transition-colors border-t border-[#E8E5DE]"
                    >
                  ...
                  <div className="p-4 text-center text-xs text-[#6B6B6B]">
                    No matching products for "{debouncedSearchQuery}"
                  </div>
                )}
              </div>
            )}
